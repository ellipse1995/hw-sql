...
select cast(EMPLOYEE_ID as varchar)+'-'+ FIRST_NAME +' ' + LAST_NAME as result from Employees where EMPLOYEE_ID=100
...
-- 3. That displays the first name and the length of the first name for all employees whose name starts with the letters 'A', 'J' or 'M'. Give each column an appropriate label. 
-- Sort the results by the employees' first names.(Employees)

select FIRST_NAME,LEN(first_name) from Employees where LEFT(first_name,1) in ('A','J','M')
...
select * from SingleOrder order by case when  id = 0 then 2 else 1 end
...
select case when ssn is not null then ssn
			when passportid is not null then passportid
			else itin end from person
