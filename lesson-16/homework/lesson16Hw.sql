...
with cte as (
select 1 as Recrusive 
union all
select Recrusive +1 from cte
where Recrusive<1000
)
...
select firstname, TotalSales from 
(select sum(Sal.salesamount) as TotalSales,Emp.firstname from Employees emp left join Sales sal on emp.EmployeeID=sal.EmployeeID group by FirstName) as Total
...
select * from (
select prod.productname, MAX(sal.salesamount) as MaxSale from sales sal left join Products prod on prod.ProductID=sal.ProductID group by prod.ProductName
) as MaxSalesAmount
...
with CTE as (
select emp.FirstName,COUNT(emp.FirstName) as SalesAmount,emp.LastName from Sales sal left join Employees emp on sal.EmployeeID=emp.EmployeeID group by emp.FirstName,emp.LastName
)
select Firstname,LastName from CTE where SalesAmount>5
...
with cte as (
select prod.ProductID,prod.ProductName,prod.Price,sal.SalesAmount from Sales Sal left join Products prod on sal.ProductID=sal.ProductID
)
select distinct Productname from cte where price*salesamount>500

-- 8 Create a CTE to find employees with salaries above the average salary.(Employees)

with cte as (
select *, (select AVG(salary) from Employees) as Avarage from Employees
)

select FirstName,LastName,Salary,Avarage from CTE where Salary>Avarage
